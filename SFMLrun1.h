...
public:
    ...
    void setPort(const string& portName, int dockSlots)
    {
        ...
        if (maxDockSlots < 1) maxDockSlots = 2;
        if (maxDockSlots > totalDockSlots) maxDockSlots = totalDockSlots;
    }

    void setDockedCount(int count)
    {
        currentlyDocked = count;
        if (currentlyDocked > maxDockSlots) currentlyDocked = maxDockSlots;
    }
...
bool isTimeValidLocal(const string& prevRouteDate, const string& prevDepTime, 
                      const string& prevArrTime, const string& nextDepDate, 
                      const string& nextDepTime)
{
    ...
    // Calculate ACTUAL arrival date (accounting for overnight voyages)
    int actualArrivalDate = dateToIntLocal(prevRouteDate);
    if (prevArrMins < prevDepMins) {
        // Overnight voyage - arrives next day
    ...
    // Case 2: Departure is after arrival (within max days)
    if (nextDate > actualArrivalDate) {
        // Calculate day difference
        int dayDiff = (nextDate - actualArrivalDate);
        // For simple check, see if it's within reasonable range
        return (dayDiff <= maxLayoverDays);
    }

    // Case 3: Departure before arrival - impossible
    return false;
}
...
bool isLayoverFeasibleLocal(const Routes& prevRoute, const Routes& nextRoute) 
{
    return isTimeValidLocal(
        prevRoute.date,       // Previous route departure date
        prevRoute.depTime,    // Previous route departure time (ADDED!)
        prevRoute.arrTime,    // Previous route arrival time
        nextRoute.date,       // Next route departure date
        nextRoute.depTime     // Next route departure time
    );
}
