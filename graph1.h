...
//route structure for graph edges
struct Route
{
    string destination;
    string date;
...
//docking ship structure for dock queue
struct DockingShip
{
    string shipName;
    string arrivalTime;
...
struct Graph
{
    ...
    //found path for storing multiple found paths
    struct FoundPath
    {
    ...
    {
        // Predefined port names with their latitudes and longitudes
        string names[40] = 
        {
    ...
    {
        // Set up docking queues for each port with specific configurations
        for (int i = 0; i < vertices; i++)
        {
    ...
    //haversine distance between two ports
    ...
    //heuristic cost and time functions
    ...
    string getNextDate(string currentDate) //returns next date in dd/mm/yyyy format
    {
        int day = 0;
        int month = 0;
        int year = 0;
    ...
    bool isTimeValid(Route* prevRoute, Route* nextRoute)
    {
        ...
        const int maxLayoverDays = 7;    // 7 days maximum
    ...
    //check if port is already in current path
    ...
    // searching every route from the current port to the destination. 
    ...
    //search for routes between source and destination. calls iterative deepening search
    ...
    //sort found paths by length using bubble sort
    ...
    void dijkstraMinCost(string source, string destination)
    {
        ...
        for (int i = count - 1; i >= 0; i--)
        {
            ...
            if (i < count - 1)
            {
                //get arrival time and date from best route
    ...
    // A* algorithm to find minimum cost path
